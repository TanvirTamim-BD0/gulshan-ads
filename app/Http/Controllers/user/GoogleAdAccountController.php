...
class GoogleAdAccountController extends Controller
{
    ...
    {   
        $accountCount = GoogleAdAccount::where('user_id',Auth::user()->id)->count();
        $buyServiceCount = BuyService::where('user_id',Auth::user()->id)->where('status','Confirmed')->count();
        $bmShareCount = GoogleAdAccountTransfer::where('user_id',Auth::user()->id)->where('status','Complete')->count();
        $disabledCount = GoogleAdAccountAppeal::where('user_id',Auth::user()->id)->where('status','Complete')->count();
        $replaceCount = GoogleAdAccountReplace::where('user_id',Auth::user()->id)->where('status','Complete')->count();
        $campaignCount = Campaign::where('user_id',Auth::user()->id)->where('status','Confirmed')->count();

        $topUpData = GoogleAdAccountTopUp::where('user_id',Auth::user()->id)->where('status','Complete')->get();
        ...
        return view('user.googleAdAccount.overview',compact('accountCount','buyServiceCount','bmShareCount','disabledCount','replaceCount','campaignCount','totalDepositAmount'));
    }

    public function adAccountRequestList()
    {   
        $accountRequestData = GoogleAdAccountRequest::where('user_id',Auth::user()->id)->get();
        return view('user.googleAdAccount.requestList',compact('accountRequestData'));
    } 

    public function createdAccount()
    {   
        $accountData = GoogleAdAccount::where('status','Created')->where('user_id',Auth::user()->id)->get();
        return view('user.googleAdAccount.createdAccount',compact('accountData'));
    }
    

    public function userAdAccountEdit($id)
    {   
        $accountData = GoogleAdAccount::where('id',$id)->first();
        return view('user.googleAdAccount.editAdAccount',compact('accountData'));
    }
    ...
    public function adAccountEditSubmit(Request $request,$id)
    {
        ...
        $data = $request->all();
        $account = GoogleAdAccount::where('id',$id)->first();

        if($account->update($data)){
            return redirect(route('google-created-account'))->with('message','Successfully Ad Account Updated');
        }else{
    ...
    {   
        $accountData = GoogleAdAccountRequest::where('id',$id)->first();
        return view('user.googleAdAccount.editAdAccountRequest',compact('accountData'));
    }
    ...
    public function adAccountRequestEditSubmit(Request $request,$id)
    {
        ...
        $data = $request->all();
        $account = GoogleAdAccountRequest::where('id',$id)->first();

        if($account->update($data)){
            return redirect(route('google-ad-account-overview'))->with('message','Successfully Ad Account Request Updated');
        }else{
            return redirect()->back()->with('error','Error !! Update Failed');;
        }
    }
    ...
    {   
        ...
        return view('user.googleAdAccount.request',compact('timeZoneData','businessTypeData'));
    }
    ...
    public function adAccountRequestSubmit(Request $request)
    {
        ...
        if(GoogleAdAccountRequest::create($data)){
    ...
    {   
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.topUp',compact('accountData'));
    }
        ...
        foreach ($request->addMoreInputFields as $key => $value) {
            ...
            if ($validatedata != '') {
                ...
                if ($userData->balance >= $totalAmount) {
                    ...
                    $check = GoogleAdAccountTopUp::where('ad_account_id',$value['ad_account_id'])->where('status','Pending')->first();

                        if ($check) {
                            
                        }else{
                            if ($value['amount'] != '') {
                                $data = new GoogleAdAccountTopUp();
                                $data->user_id = Auth::user()->id;
    ...
    {
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.foundTransfer',compact('accountData'));
    }
     ...
     public function adAccountFoundTransferSubmit(Request $request)
    {
        ...
        if(GoogleAdAccountFoundTransfer::create($data)){
    ...
    {   
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.trasnfer',compact('accountData'));
    }
            ...
            if ($validatedata != '') {
                foreach ($request->addMoreInputFields as $key => $value) {
                    if ($value['business_manager_id'] != '' && isset($value['transfer_or_share'])) {
                        $data = new GoogleAdAccountTransfer();
                        $data->user_id = Auth::user()->id;
                        $data->ad_account_id = $value['ad_account_id'];
    ...
    {   
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.appeal',compact('accountData'));
    }
    ...
    {   
        ...
        foreach($request->ad_account_ids as $ad_account_id){
            $data = new GoogleAdAccountAppeal();
            $data->user_id = Auth::user()->id;
    ...
    {   
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.replace',compact('accountData'));
    }
            ...
            if ($validatedata != '') {
                foreach ($request->addMoreInputFields as $key => $value) {
                    if ($value['business_manager_id'] != '' && isset($value['screenshot_of_rejected_appeal']) ) {
                        $data = new GoogleAdAccountReplace();
                        $data->user_id = Auth::user()->id;
                        $data->ad_account_id = $value['ad_account_id'];
                        $data->business_manager_id = $value['business_manager_id'];
    ...
    {   
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.renameRequest',compact('accountData'));
    }
    ...
    public function adAccountRenameRequestSubmit(Request $request)
    {
        ...
        if(GoogleAdAccountRenameRequest::create($data)){
    ...
    {
        return view('user.googleAdAccount.bmLinkRequest');
    }
    ...
    public function adAccountBmLinkRequestSubmit(Request $request)
    {
        ...
        $data = new GoogleAdAccountBMLinkRequest();
        $data->user_id = Auth::user()->id;
    ...
    {   
        $accountData = GoogleAdAccount::where('status','Created')->where('user_id',Auth::user()->id)->get();
        return view('user.googleAdAccount.refundRequest',compact('accountData'));
    }
    ...
    public function adAccountRefundRequestSubmit(Request $request)
    {
        ...
        $data = new GoogleAdAccountRefundRequest();
        $data->user_id = Auth::user()->id;
    ...
    {   
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.tryHold',compact('accountData'));
    }
    ...
    {   
        $accountData = GoogleAdAccount::where('user_id',Auth::user()->id)->where('status','Created')->get();
        return view('user.googleAdAccount.billFailed',compact('accountData'));
    }
    ...
    {   
        ...
        foreach($request->ad_account_ids as $ad_account_id){
            $data = new GoogleBillFailedRequest();
            $data->user_id = Auth::user()->id;
            $data->ad_account_id = $ad_account_id;
            $data->save();
        }
